...
function Detail({ item }: { item: ApiListing }) {
  ...
  return <section className="space-y-5"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="flex gap-2"><button type="button" className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700">Düzenle</button><button type="button" className="rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white">İlanı Görüntüle</button></div></div>
...
export default function PortfolioPage() {
  ...
  const [query,setQuery]=useState(""); const [purpose,setPurpose]=useState<(typeof purposeFilters)[number]>("Tümü"); const [type,setType]=useState<(typeof typeFilters)[number]>("Tümü"); const [selectedId,setSelectedId]=useState<string|null>(null); const [loading,setLoading]=useState(true); const [error,setError]=useState("");
  async function load() {
